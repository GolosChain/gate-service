...
class Broker extends BasicService {
    ...
    async start() {
        ...
        await front.start(async ({ channelId, ip }, data, pipe) => {
            if (R.is(String, data)) {
                await this._handleFrontendEvent(channelId, data, pipe);
            } else {
                await this._handleRequest({ channelId, ip }, data, pipe);
            }
        });
    ...
    async _handleRequest({ channelId, ip }, data, pipe) {
        ...
        if (this._userMapping.get(channelId) === null) {
            await this._handleAnonymousRequest({ channelId, ip }, data, pipe);
        } else {
            await this._handleClientRequest({ channelId, ip }, data, pipe);
        }
    }
    ...
    async _handleAnonymousRequest({ channelId, ip }, data, pipe) {
        switch (data.method) {
            case 'getSecret':
    ...
    async _handleClientRequest({ channelId, ip }, data, pipe) {
        const translate = this._makeTranslateToServiceData({ channelId, ip }, data);
    ...
    _makeTranslateToServiceData({ channelId, ip }, data) {
        return {
            _frontendGate: true,
            ip, 
            channelId,
